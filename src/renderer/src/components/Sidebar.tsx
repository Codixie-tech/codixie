...
import { useRemoveTag, useUpdateTag } from '@/hooks/tag';
...
const DefaultTagElement = ({ text }: { text: DateFiltersType }) => {
  ...
  return (
    <Button variant={currentDateFilter === text ? 'tagSelected' : 'tag'}
      className="flex max-w-full items-center justify-start gap-3"
      onClick={() => setCurrentDateFilter(text)} size="tag">
...
const CustomTagList = () => {
  ...
  const sortedTags = useMemo(() => tags.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()), [tags]);

  return (
    <div ref={animationParent}>
      <div className="flex items-center justify-between py-1 pl-4">
        <h1 className="pl-1 text-sm font-semibold text-gray-5">Tags</h1>
        <Button onClick={() => showModal({ modalType: 'CREATE_TAG_MODAL' })} className="h-8 w-8 md:hidden" variant="styleLess" size="icon">
          <i className="ri-add-line ri-lg min-w-7" />
        </Button>
      </div>
      {sortedTags.map((tag) => <CustomTagElement key={tag.id} tag={tag} />)}
    </div>
  );
};
...
const DownloadUploadButtons = () => {
  ...
  return (
    <>
      <Button asChild size="icon" className="flex-0"
      ...
      <Button onClick={handleExport} size="icon" className="flex-0"
  ...
  return (
    <div className="flex h-screen flex-col">
      <ScrollArea className="flex-1 bg-gray-3 dark:bg-dark-gray-7">
        <section className="block h-full w-full overflow-hidden overflow-x-hidden px-3 pt-2 md:pt-0">
          <div className="flex items-center pt-[9px] md:h-[60px] md:pt-0">
            <Button className="mr-8 max-w-full flex-1 md:mr-auto"
              onClick={() => showModal({ modalType: 'CREATE_TAG_MODAL' })}>
              Create tag
            </Button>
            <Button variant="styleLess" size="icon" className="ml-auto h-8 w-8 md:hidden"
      ...
      <div className="flex h-[60px] items-center justify-start gap-2 bg-gray-3 px-4 dark:bg-dark-gray-7">
        <Button size="icon" className="flex-0 flex justify-center"
